...
from utils import merge_vggt_latents
...
class LatentLeRobotDataset(LeRobotDataset):
    def __init__(
        ...
        self.vggt_latent_path = Path(repo_id) / 'vggt_latents'
    ...
    def _check_meta(self, start_frame, end_frame, episode_index):
        ...
        vggt_latent_file = (
            Path(self.vggt_latent_path) / f"chunk-{episode_chunk:03d}" /
            f"episode_{episode_index:06d}_{start_frame}_{end_frame}.pth"
        )
        if not os.path.exists(vggt_latent_file):
            return False
    ...
    def _get_range_latent_data(self, start_frame, end_frame, episode_index):
        ...
        assert os.path.exists(vggt_latent_file)
        out["vggt"] = torch.load(vggt_latent_file, weights_only=False)
    ...
    def _get_vggt_frame_mode(self, data_dict):
        frame_mode = data_dict.get("vggt.vggt_latent_frame_mode", None)
        if frame_mode is None:
            frame_mode = getattr(self.config, "vggt_latent_frame_mode", None)
        return frame_mode

    def _cat_vggt_latents(self, data_dict, target_num_frames):
        vggt_latents = data_dict["vggt.vggt_latents"]
        vggt_latent_num_frames = data_dict["vggt.vggt_latent_num_frames"]
        vggt_latent_height = data_dict["vggt.vggt_latent_height"]
        vggt_latent_width = data_dict["vggt.vggt_latent_width"]
        vggt_latents = rearrange(
            vggt_latents,
            '(f h w) c -> f h w c',
            f=vggt_latent_num_frames,
            h=vggt_latent_height,
            w=vggt_latent_width,
        )
        return merge_vggt_latents(
            vggt_latents,
            target_num_frames=target_num_frames,
            frame_mode=self._get_vggt_frame_mode(data_dict),
        )
    ...
    def __getitem__(self, idx) -> dict:
        ...
        out_dict['vggt_latents'] = self._cat_vggt_latents(
            ori_data_dict,
            target_num_frames=out_dict['latents'].shape[0],
        )
        ...
        out_dict['vggt_latents'] = out_dict['vggt_latents'].permute(3, 0, 1, 2)
    ...
    def random_crop_n_dim(self, out_dict):
        ...
        for key in ["latents", "vggt_latents", "actions", "actions_mask"]:
            # 3. 执行切片
